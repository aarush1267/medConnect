...
// Handle new meeting POST
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_meeting'])) {
    $meeting_link = mysqli_real_escape_string($connection, $_POST['meeting_link']);
    $notes = mysqli_real_escape_string($connection, $_POST['notes']);
    $consultation_id = intval($_POST['consultation_id']);
    $consultant_id = $_SESSION['id'];
    $user_id = intval($_POST['user_id']);

    $insert_meeting = $connection->prepare("INSERT INTO consultation_meetings (consultation_id, consultant_id, user_id, meeting_link, notes) VALUES (?, ?, ?, ?, ?)");
    $insert_meeting->bind_param("iiiss", $consultation_id, $consultant_id, $user_id, $meeting_link, $notes);
    $insert_meeting->execute();

    header("Location: consultant_window.php?consultation_id={$consultation_id}&section=meeting");
    exit;
}

// Fetch meetings for this consultation
$meeting_stmt = $connection->prepare("SELECT id, meeting_link, notes, created_at FROM consultation_meetings WHERE consultation_id = ?");
$meeting_stmt->bind_param("i", $consultation_id);
$meeting_stmt->execute();
$meeting_result = $meeting_stmt->get_result();

// Delete meeting if requested
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_meeting'])) {
    $meeting_id = intval($_POST['meeting_id']);
    $stmt = $connection->prepare("DELETE FROM consultation_meetings WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['end_consultation'])) {
    ...
    $stmt = $connection->prepare("UPDATE consultations SET status = 'completed' WHERE id = ? AND consultant_id = ?");
    $stmt->bind_param("ii", $consultation_id, $_SESSION['id']);
    $stmt->execute();

    header("Location: consultant_consult.php"); // redirect to refresh and move to Previous Consultations
    exit;
}
  ...
  <div id="meetingSection" class="consult-window-section" style="display: none;">
      <div class="meeting-card">
          ...
          <form method="POST" class="meeting-form">
              ...
              <label for="notes">Notes for the Meeting (optional)</label>
              ...
              <button type="submit" name="submit_meeting">Schedule Meeting</button>
          </form>
      </div>
          ...
          <div class="meeting-list">
              <?php while ($meeting = $meeting_result->fetch_assoc()): ?>
                  <div class="meeting-item">
                      ...
                      <?php endif; ?>
                      <p class="meeting-timestamp">Date: <?= date("F j, Y, g:i a", strtotime($consultation['date'])) ?></p>
                      ...
                      <div class="meeting-buttons">
                        ...
                        <!-- Delete Button -->
                        <form method="POST" onsubmit="return confirm('Are you sure you want to delete this meeting?');" style="margin-top: 10px;">
                            <input type="hidden" name="meeting_id" value="<?= $meeting['id'] ?>">
                            <button type="submit" name="delete_meeting" class="delete-meeting-btn">Delete Meeting</button>
                        </form>
                      </div>
                  </div>
              <?php endwhile; ?>
      ...
      <?php else: ?>
          <p class="no-meetings">No meetings have been scheduled yet for this consultation.</p>
      <?php endif; ?>
  </div>
